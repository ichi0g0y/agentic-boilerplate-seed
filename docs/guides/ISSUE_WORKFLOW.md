...
- `.context/issue_scope.json` で対象Issueを共有してよい
- `/pick` / `/p` は任意。計画段階では未設定でもよい
- `.context` 未設定時はIssue固定フローを使わず通常動作で進める
...
6. `.context` が未設定ならIssue番号を依頼文で明示して進める
7. 着手時に `status:in-progress` を付与する
8. 実装・テストを行い、必要に応じてIssueコメントで進捗共有する
9. PR本文に `Closes #<issue-number>` を記載してマージする
10. マージでIssueを自動クローズする
...
- Issueが確定した場合は `primary_issue` と `related_issues` のレビューコメントを収集する
...
- Codex:
  ...
  - `/p` `/rv` `/c` など短縮文字列だけを送らず、処理内容を文章で明示する
  - 例:
