...
- コミット実行の唯一の条件は、ユーザー最新指示に `/commit` 系コマンドが明示されていること
...
- 完全な絵文字リストは `.claude/commands/commit.md` を参照する
...
- ステージングの可否をユーザーに確認しない
- ステージ後に `git diff --cached --name-only` で対象ファイルを確認する
